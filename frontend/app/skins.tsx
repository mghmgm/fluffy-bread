...
export default function Skins() {
  ...
  const { ownedSkins, activeSkin, setActiveSkin } = useSkins();
  ...
  const handleSkinSelect = async (id: SkinId) => {
    try {
      ...
      console.warn('Failed to set active skin:', error);
    }
  };

  // Исправлено: добавлена обработка случая, когда ownedSkins может быть undefined
  const availableSkins = (ownedSkins || ['bread_default']) as SkinId[];
          ...
          {availableSkins.map((id) => (
            ...
            >
              ...
              <RNText style={styles.skinLabel}>
                {id === 'bread_default' ? 'Обычный' : id === 'bread_alt' ? 'Альтернативный' : id}
              </RNText>
            </TouchableOpacity>
          ))}
        </View>
